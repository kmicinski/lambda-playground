...
var DerivationTree = class {
  nodes = /* @__PURE__ */ new Map();
  displayIndex = /* @__PURE__ */ new Map();
  rootId = null;
  currentId = null;
  navHistory = [];
  nextId = 0;
  setRoot(displayStr, renderTreeJson) {
    this.nodes.clear();
  ...
  addChild(displayStr, opLabel, renderTreeJson, redexPath) {
    ...
    const id = this.nextId++;
    const node = { id, displayStr, renderTreeJson, parentIds: [parentId], children: [] };
...
var examplesSelect = $("#examples-select");
var randomBtn = $("#random-btn");
var closedCheck = $("#closed-check");
var undoBtn = $("#undo-btn");
var viewport = $("#viewport");
var termContainer = $("#term-container");
var termDisplay = $("#term-display");
var zoomInBtn = $("#zoom-in");
var zoomOutBtn = $("#zoom-out");
var zoomResetBtn = $("#zoom-reset");
var zoomLevel = $("#zoom-level");
var contextMenu = $("#context-menu");
var alphaPopup = $("#alpha-popup");
var alphaOld = $("#alpha-old");
var alphaNameInput = $("#alpha-name");
var strategyBtns = document.querySelectorAll(".strategy-btn");
var stepBtn = $("#step-btn");
var runBtn = $("#run-btn");
var stopBtn = $("#stop-btn");
var stepCountEl = $("#step-count");
var normalFormBadge = $("#normal-form-badge");
var statusSize = $("#term-size");
var statusFv = $("#term-fv");
var statusRedex = $("#redex-count");
var statusStrategy = $("#strategy-name");
var treePanel = $("#tree-panel");
var treeView = $("#tree-view");
var treeToggleBtn = $("#tree-toggle");
var drawer = $("#drawer");
...
function navigateTree(key) {
  ...
  if (key === "ArrowUp") {
    ...
    const parent = cur.parentIds.length > 0 ? dtree.nodes.get(cur.parentIds[0]) : void 0;
    if (parent) {
...
function setupEventListeners() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!e.metaKey && !e.ctrlKey && !e.altKey && notTyping) {
      ...
      if (e.key === " ") {
        ...
        if (stepBtn.disabled) {
          try {
            const info = JSON.parse(engine.get_term_info());
            const strat = engine.get_strategy();
            const stratNames = { normal: "normal order", applicative: "applicative order", cbv: "call-by-value", cbn: "call-by-name" };
      ...
      const stratKeys = { n: "normal", a: "applicative", v: "cbv", l: "cbn" };
  ...
  alphaNameInput.addEventListener("keyup", (e) => e.stopPropagation());
  document.addEventListener("click", (e) => {
    if (!alphaPopup.contains(e.target)) hideAlphaPopup();
  });
...
function renderCurrentTerm() {
  ...
  const childHighlights = /* @__PURE__ */ new Map();
  ...
  for (const [nodeId, dnode] of dtree.nodes) {
    ...
    if (isCurrent) {
      ...
      if (dnode.renderTreeJson) {
        try {
          const staticTree = JSON.parse(dnode.renderTreeJson);
  ...
  for (const id of dtree.nodes.keys()) computeDepth(id, /* @__PURE__ */ new Set());
  const layers = [];
  ...
  if (layers.length > 0) {
    ...
    for (let d = 1; d < layers.length; d++) {
      ...
      for (const id of ids) {
        ...
        const parentXs = node.parentIds.map((pid) => xPositions.get(pid)).filter((x) => x != null);
  ...
  const yPositions = /* @__PURE__ */ new Map();
  let yAccum = pad;
  ...
  for (const [id, color] of [["main-arrow", "#FDDCB5"], ["main-arrow-active", "#F76900"]]) {
  ...
  for (const node of dtree.nodes.values()) {
    ...
    for (const child of node.children) {
      ...
      if (child.id === node.id) {
        ...
        path2.setAttribute(
          "d",
          `M${x0},${y0 + parentH / 2 - 4} C${x0 + loopR + 20},${y0 + parentH / 2 + loopR} ${x0 + loopR + 20},${y0 - parentH / 2 - loopR} ${x0},${y0 - parentH / 2 + 4}`
        );
        path2.setAttribute("stroke", "#F76900");
        path2.setAttribute("stroke-width", "2.5");
        path2.setAttribute("fill", "none");
        path2.setAttribute("stroke-dasharray", "6,3");
        path2.setAttribute("marker-end", "url(#main-arrow-active)");
...
function flatWidth(node) {
  ...
  switch (node.kind) {
    case "var":
      node._fw = (node.name ?? "").length;
      break;
    case "abs":
      node._fw = 7 + (node.param ?? "").length + flatWidth(node.body);
      break;
    case "app":
      node._fw = 3 + flatWidth(node.func) + flatWidth(node.arg);
      break;
    default:
      node._fw = 0;
  }
...
function renderNode(node, maxCols, col) {
...
function renderNodeStatic(node, highlightPath, maxCols, col) {
  const span = document.createElement("span");
  span.className = `term term-${node.kind}`;
...
function highlightBinding(absPath, hoveredElement) {
  const scope = document.getElementById("current-term-card") || termDisplay;
...
function drawBindingArrow(absPath, hoveredElement) {
  removeBindingArrow();
  const card = document.getElementById("current-term-card");
  if (!card) return;
  const binder = card.querySelector(`.var-binding[data-abs-path="${absPath}"]`);
  if (!binder) return;
  const refs = card.querySelectorAll(`.var-ref[data-binder-path="${absPath}"]`);
...
function showContextMenu(x, y, ops) {
  ...
  for (const op of ops) {
    ...
    btn.addEventListener("click", (e) => {
      ...
      const ax = rect2.left + rect2.width / 2;
      const ay = rect2.top;
      hideContextMenu();
...
function handleOperation(op, path, alphaTarget, anchorX, anchorY) {
  if (op === "alpha") {
...
function showAlphaPopup(target, anchorX, anchorY) {
  alphaNameInput.value = "";
  alphaPopup.dataset["targetPath"] = target;
  const scope = document.getElementById("current-term-card") || termDisplay;
  const absEl = scope.querySelector(`[data-path="${target}"]`);
...
function renderTreeNode(nodeId, visited) {
  const node = dtree.nodes.get(nodeId);
  const isCurrent = nodeId === dtree.currentId;
...
function centerOnCurrent(smooth) {
  ...
  if (smooth) {
    ...
    updateTransform();
    const clear = () => {
      termContainer.style.transition = "none";
    };
...
function updateStatusBar(info) {
  ...
  statusFv.textContent = info.free_vars.length > 0 ? `FV: {${info.free_vars.join(", ")}}` : "Closed term";
...
function flashError(raw) {
  ...
  el.textContent = msg;
  document.body.appendChild(el);
  const displayTime = Math.max(2500, Math.min(msg.length * 40, 6e3));
  setTimeout(() => {
    el.style.opacity = "0";
  }, displayTime);
  setTimeout(() => {
...
var tutorialStep = -1;
var tutorialEl = $("#tutorial");
var tutorialBody = $("#tutorial-body");
var tutorialIndicator = $("#tutorial-indicator");
var tutorialDots = $("#tutorial-dots");
var tutorialPrev = $("#tutorial-prev");
var tutorialNext = $("#tutorial-next");
var tutorialCloseBtn = $("#tutorial-close");
var tutorialStartBtn = $("#tutorial-btn");
var tutorialFab = $("#tutorial-fab");
...
function showTutorialExitPrompt() {
  ...
  if (step.autoSteps) {
    let doAutoStep2 = function() {
      ...
      setTimeout(() => {
        ...
        if (count < total) setTimeout(doAutoStep2, 1200);
      }, 400);
    };
    var doAutoStep = doAutoStep2;
    const indicator = document.getElementById("tut-space-indicator");
    let count = 0;
    const total = step.autoSteps;
    setTimeout(doAutoStep2, 800);
  }
  if (step.crDemo) {
    setTimeout(() => runChurchRosserDemo(step), 600);
  }
}
function runChurchRosserDemo(step) {
  ...
  function phase2Done(nf1) {
    if (!stillActive()) return;
    const nf2 = engine.get_display();
    const converges = nf1 === nf2;
    const result = document.createElement("div");
    result.className = "tut-cr-result" + (converges ? " converges" : "");
    result.innerHTML = converges ? `Both paths arrived at <strong><code>${nf2}</code></strong> \u2014 Church-Rosser confirmed! \u2714` : `Normal order got <code>${nf1}</code>, Applicative got <code>${nf2}</code>.`;
    statusEl.innerHTML = "";
...
function showWelcomeNudge() {
  try {
    sessionStorage.setItem("lambda_welcomed", "1");
  } catch (_) {
  }
  const el = document.createElement("div");
  ...
  const autoTimer = setTimeout(dismiss, 4e3);
